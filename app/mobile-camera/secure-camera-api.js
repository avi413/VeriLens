...
class SecureCameraAPI {
  ...
  async setupSecureCapture() {
    ...
    this.secureConstraints = {
      video: {
        ...
        zoom: { ideal: 1.0, max: 1.0 }, // Prevent digital zoom
  ...
  async performPreCaptureSecurityCheck() {
    const checks = {
      ...
      frameRate: this.getActualFrameRate() > 10,
      motionDetected: this.hasRecentMotion(),
      noExternalSource: !this.detectExternalVideoSources().external,
      antiTampering: !window.antiTamperingService?.tamperingDetected,
    };
